...
namespace fasttris {
namespace {
void setError(std::string* err,const std::string& message){if(err)*err=message;}

void writeReplay(std::ostream& f,const Replay&r){
    f<<"FASTTRIS_REPLAY "<<r.version<<"\nseed "<<r.seed<<"\nmode "<<static_cast<int>(r.mode)<<"\n";
    const auto&h=r.rules.handling;
    f<<"das "<<h.das_ms<<"\narr "<<h.arr_ms<<"\nsdf "<<h.sdf<<"\ndcd "<<h.dcd_ms
    ...
    for(const auto&e:r.events)f<<"event "<<e.time_us<<' '<<actionToken(e.action)<<' '<<(e.down?1:0)<<"\n";
...
bool deserializeReplay(std::string_view text,Replay&out,std::string*err){
    out=Replay{};
    std::istringstream f{std::string(text)};
    std::string line;
    if(!std::getline(f,line)){setError(err,"empty replay");return false;}
    std::istringstream head(line);
    std::string magic;
    head>>magic>>out.version;
    if(!head||magic!="FASTTRIS_REPLAY"||out.version!=1){setError(err,"unsupported replay format");return false;}
    ...
    while(std::getline(f,line)){
        ...
        if(k=="seed"){s>>out.seed;seen|=Seed;}
            ...
            int v;s>>v;
            if(v<0||v>static_cast<int>(Mode::Custom)){setError(err,"invalid mode");return false;}
            ...
            s>>e.time_us>>a>>d;
            if(!s||!parseActionToken(a,e.action)){setError(err,"bad replay event");return false;}
        ...
        else {setError(err,"unsupported replay field");return false;}
        if(!s){setError(err,"malformed replay field");return false;}
    }

    if(seen!=required){setError(err,"incomplete replay format");return false;}
    return true;
}
...
bool loadReplay(const std::string&path,Replay&out,std::string*err){
    ...
    data<<f.rdbuf();
    if(!f.good()&&!f.eof()){setError(err,"failed reading replay");return false;}
    return deserializeReplay(data.str(),out,err);
}
