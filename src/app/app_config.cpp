...
void clampConfig(AppConfig&c){
    auto&h=c.rules.handling;h.das_ms=std::clamp(h.das_ms,0,1000);h.arr_ms=std::clamp(h.arr_ms,0,500);h.sdf=std::clamp(h.sdf,0,200);h.dcd_ms=std::clamp(h.dcd_ms,0,1000);h.lock_delay_ms=std::clamp(h.lock_delay_ms,0,2000);h.max_lock_resets=std::clamp(h.max_lock_resets,0,100);c.rules.next_count=std::clamp(c.rules.next_count,1,8);c.fps_cap=std::clamp(c.fps_cap,0,1000);
}
}
...
bool loadConfig(const std::string&path,AppConfig&c){
    std::ifstream f(path);if(!f)return false;std::string line;while(std::getline(f,line)){auto p=line.find('=');if(p==std::string::npos)continue;auto k=line.substr(0,p),v=line.substr(p+1);try{
        int n=std::stoi(v);auto&h=c.rules.handling;if(k=="das")h.das_ms=n;else if(k=="arr")h.arr_ms=n;else if(k=="sdf")h.sdf=n;else if(k=="dcd")h.dcd_ms=n;else if(k=="lock")h.lock_delay_ms=n;else if(k=="resets")h.max_lock_resets=n;else if(k=="allow180")h.allow_180=n!=0;else if(k=="ghost")c.rules.ghost=n!=0;else if(k=="vsync")c.vsync=n!=0;else if(k=="fps")c.fps_cap=n;else if(k=="tournament")c.rules.tournament=n!=0;else if(k.rfind("key",0)==0){int i=std::stoi(k.substr(3));if(i>=0&&i<(int)c.keys.size())c.keys[i]=static_cast<SDL_Keycode>(n);}else if(k.rfind("pad",0)==0){int i=std::stoi(k.substr(3));if(i>=0&&i<(int)c.pads.size())c.pads[i]=n;}
    }catch(...){}}
    clampConfig(c);return true;
}
bool saveConfig(const std::string&path,const AppConfig&c){
    std::ofstream f(path);if(!f)return false;const auto&h=c.rules.handling;f<<"das="<<h.das_ms<<"\narr="<<h.arr_ms<<"\nsdf="<<h.sdf<<"\ndcd="<<h.dcd_ms<<"\nlock="<<h.lock_delay_ms<<"\nresets="<<h.max_lock_resets<<"\nallow180="<<h.allow_180<<"\nghost="<<c.rules.ghost<<"\nvsync="<<c.vsync<<"\nfps="<<c.fps_cap<<"\ntournament="<<c.rules.tournament<<"\n";for(std::size_t i=0;i<c.keys.size();++i)f<<"key"<<i<<'='<<static_cast<long long>(c.keys[i])<<"\n";for(std::size_t i=0;i<c.pads.size();++i)f<<"pad"<<i<<'='<<c.pads[i]<<"\n";return bool(f);
}
