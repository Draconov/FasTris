...
void renderGame(SDL_Renderer*r,Game&g,const RenderInfo&i){
    ...
    switch(g.mode()){
        ...
        case Mode::Finesse:{
            ...
            txtf(r,20,390,{145,155,170,255},"FINESSE %.1f%%",acc);
            txtf(r,20,408,{130,210,255,255},"FAULTS %d  STREAK %d",s.finesse_faults,s.finesse_streak);
            break;
        }
        case Mode::Zen:
        ...
        case Mode::Custom:{
            ...
            if(cr.custom_line_goal>0)txtf(r,20,390,{145,155,170,255},"LINES LEFT %d",std::max(0,cr.custom_line_goal-s.lines));
            else txt(r,20,390,"LINES  ENDLESS",{145,155,170,255});
            if(cr.custom_time_limit_s>0)txtf(r,20,408,{130,210,255,255},"TIME LEFT %.3f",std::max(0.0,double(cr.custom_time_limit_s)-sec));
            else if(cr.custom_gravity_ms==0)txt(r,20,408,"GRAVITY OFF",{130,210,255,255});
            else txtf(r,20,408,{130,210,255,255},"GRAVITY %d MS/CELL",cr.custom_gravity_ms);
            break;
        }
    }
    ...
    if(g.gameOver()){
        ...
        fill(r,330,268,300,160,{5,8,12,235});txt(r,385,286,"RUN COMPLETE",{120,240,170,255},true);
        switch(g.mode()){
            case Mode::Sprint40:
            case Mode::SeedRace:
                txtf(r,374,326,{220,225,232,255},"TIME %.3f   PPS %.2f",sec,pps);
                txtf(r,374,348,{220,225,232,255},"KPP %.2f   FIN %d",kpp,s.finesse_faults);
                break;
            case Mode::Ultra120:
                txtf(r,374,326,{220,225,232,255},"SCORE %lld",(long long)s.score);
                txtf(r,374,348,{220,225,232,255},"LINES %d   PPS %.2f",s.lines,pps);
                break;
            case Mode::Marathon:
                txtf(r,374,326,{220,225,232,255},"SCORE %lld",(long long)s.score);
                txtf(r,374,348,{220,225,232,255},"TIME %.3f   LINES %d",sec,s.lines);
                break;
            case Mode::Cheese40:
                txtf(r,374,326,{220,225,232,255},"TIME %.3f   PPS %.2f",sec,pps);
                txtf(r,374,348,{220,225,232,255},"CHEESE %d   FIN %d",s.garbage_lines_cleared,s.finesse_faults);
                break;
            case Mode::Custom:
                txtf(r,374,326,{220,225,232,255},"SCORE %lld   LINES %d",(long long)s.score,s.lines);
                txtf(r,374,348,{220,225,232,255},"TIME %.3f   PPS %.2f",sec,pps);
                break;
            case Mode::Finesse:
            case Mode::Zen:
                txtf(r,374,326,{220,225,232,255},"TIME %.3f   PIECES %d",sec,s.pieces);
                txtf(r,374,348,{220,225,232,255},"FIN %d   PPS %.2f",s.finesse_faults,pps);
                break;
        }
        txt(r,382,390,"F5 RETRY   ESC MENU",{180,190,205,255});
    }
}
